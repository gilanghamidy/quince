...
exprn_mapper<std::optional<double>> avg(const abstract_mapper<int16_t> &);
exprn_mapper<std::optional<double>> avg(const abstract_mapper<int32_t> &);
exprn_mapper<std::optional<double>> avg(const abstract_mapper<int64_t> &);
exprn_mapper<std::optional<double>> avg(const abstract_mapper<float> &);
exprn_mapper<std::optional<double>> avg(const abstract_mapper<double> &);
...
template<typename T>
exprn_mapper<std::optional<T>>
max(const abstract_mapper<T> &m) {
    return function<std::optional<T>>("max", m);
}

template<typename T>
exprn_mapper<std::optional<T>>
min(const abstract_mapper<T> &m) {
    return function<std::optional<T>>("min", m);
}
